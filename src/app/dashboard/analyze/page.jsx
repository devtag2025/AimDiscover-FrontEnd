  ...
  return (
    <>
      ...
      <spotLight
        ...
        castShadow
      />
      <pointLight position={[-10, -10, -10]} intensity={0.5} color="#9333ea" /> {/* Purple backlight */}
      ...
      <div className="w-full h-[500px] bg-[#0A0A0A] rounded-2xl border border-white/5 flex flex-col items-center justify-center relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col items-center max-w-md px-6 text-center">
          ...
          <p className="text-purple-200/60 text-sm mb-6">
            {modelData?.message || "Synthesizing 3D geometry and PBR textures..."}
          </p>
          ...
          <div className="flex items-center gap-4 mt-4 text-xs font-mono text-gray-500">
            <span>STAGE: <span className="text-purple-400 uppercase">{modelData?.stage || "INIT"}</span></span>
            <span>|</span>
            <span>STATUS: <span className="text-blue-400">{modelData?.status || "WAITING"}</span></span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="w-full h-[500px] bg-[#0A0A0A] rounded-2xl border border-red-500/20 flex items-center justify-center">
        <div className="text-center p-6">
          ...
          <p className="text-gray-500 text-sm">{modelData.taskError || "System encountered an error"}</p>
        </div>
      </div>
    );
  }
              ...
              <section className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center justify-between mb-4 px-1">
                  <h2 className="text-xl font-bold text-white flex items-center gap-2">
                    <span className="text-purple-500"><Icons.Cube /></span> 3D Prototype
                  </h2>
            ...
            {!analysisResult && !isAnalyzing && (
              <div className="h-[400px] flex flex-col items-center justify-center text-center border-2 border-dashed border-white/5 rounded-3xl bg-white/[0.02]">
                ...
                <p className="text-gray-500 max-w-xs text-sm">
                  Configure your product parameters on the left to generate market insights and 3D visualizations.
                </p>
              </div>
            )}
          </div>
